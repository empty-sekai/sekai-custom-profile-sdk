...
export class SemanticWebglSceneRenderer {
  ...
  private retainedScene: {
    ...
    imageSources: Map<string, TexImageSource>;
  ...
  async setScene(input: {
    ...
    const atlasUpload = await this.executor.setSdfAtlas(input.atlas);
    ...
    this.numericRegions = textOperations.flatMap((operation) => buildNumericTextRegions(
      ...
      input.layout.instances,
      operation.visible && operation.commandVisible,
    ));
    const glyphsByCommand = compileSemanticTextGlyphBatches(input.layout.instances, textOperations);
  ...
  async restoreContext(gl: WebGL2RenderingContext): Promise<{
    ...
    try {
      ...
      const atlasUpload = await restored.setSdfAtlas(retained.atlas);
