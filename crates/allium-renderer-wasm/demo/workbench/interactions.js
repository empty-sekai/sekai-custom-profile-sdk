...
export class InteractionOverlay {
  ...
  render(dump) {
    ...
    for (const region of this.regions) {
      ...
      polygon.addEventListener("wheel", (event) => {
        ...
        if (!binding) return;
        event.preventDefault();
        this.actions.onScroll?.(binding.control_id, Math.sign(event.deltaY) * Number(binding.step ?? 1), region);
      }, { passive: false });
  ...
  updateMode() {
    this.svg.hidden = !this.enabled && !this.selectedId;
  ...
  async activate(region) {
    ...
    this.actions.onEvent?.({ type: "activate", region, value: resolvedTarget(region) });
  }
...
export function scrollBinding(bindings) {
  return (bindings ?? []).find((binding) => binding.kind === "scroll");
}
