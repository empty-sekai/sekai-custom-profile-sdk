export class TelemetryPanel {
  ...
  render(stats) {
    ...
    const resourceProvider = resources.provider ?? {};
    const fontProvider = stats?.renderer?.fonts?.provider ?? {};
    const localizationProvider = stats?.renderer?.localizations ?? {};
    ...
    const groups = [
      ...
      },
      {
        title: "Providers",
        metrics: [
          metric("Resource calls", resourceProvider.requested),
          metric("Resource loaded", resourceProvider.loaded),
          metric("Resource failures", resourceProvider.failures, "", Number(resourceProvider.failures) === 0 ? "good" : "warn"),
          metric("Resource cancellations", resourceProvider.cancellations),
          metric("Resource encoded", resourceProvider.encodedBytes, "B"),
          metric("Resource resolve", resourceProvider.resolveMs, "ms"),
          metric("Resource peak", resourceProvider.peak),
          metric("Font demands", fontProvider.requested),
          metric("Fonts loaded", fontProvider.loaded),
          metric("Font bytes", fontProvider.bytes, "B"),
          metric("Font failures", fontProvider.failures, "", Number(fontProvider.failures) === 0 ? "good" : "warn"),
          metric("Font resolve", fontProvider.resolveMs, "ms"),
          metric("Font peak", fontProvider.peakActive),
          metric("Localization demands", localizationProvider.requested),
          metric("Localized values", localizationProvider.resolved),
          metric("Localization failures", localizationProvider.failures, "", Number(localizationProvider.failures) === 0 ? "good" : "warn"),
          metric("Localization resolve", localizationProvider.resolveMs, "ms"),
          metric("Localization peak", localizationProvider.peakActive),
        ],
      },
      {
        title: "Semantic core",
