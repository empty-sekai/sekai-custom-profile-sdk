...
fn write_asset_audit(output: &Path, plan: &HonorPlan, missing: &[String]) -> Result<(), String> {
    ...
    let entries = missing
        ...
        .map(|key| {
            serde_json::json!({
                "key": key,
                "logical_path": sekai_profile_renderer::asset_keys::key_to_s3_path(key, "assets/cn/"),
            })
        })
        .collect::<Vec<_>>();
...
fn render_object(
    ...
    match object.recipe {
        HonorObjectRecipe::Standard {
            ...
            honor_level,
            full_size,
        } => sekai_profile_renderer::elements::honor::render_static_honor(
            surface.canvas(),
            honor_id,
            honor_level,
            full_size,
        ...
        HonorObjectRecipe::Bonds {
            ...
            inverse,
            use_unit_virtual_singer,
        } => sekai_profile_renderer::elements::honor::render_bonds_honor(
            surface.canvas(),
            honor_id,
            honor_level,
            full_size,
