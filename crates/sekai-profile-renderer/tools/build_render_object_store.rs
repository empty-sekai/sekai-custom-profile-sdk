...
use sekai_profile_renderer::compiled_profile::{CompiledProfileBatch, CompiledResourceRequest};
use sekai_profile_renderer::render_object::{
    MappedRenderObjectStore, RenderObjectKind, RenderObjectManifest, RenderObjectStoreWriter,
    RenderObjectWrite,
};
use sekai_profile_renderer::render_object_catalog::{
        ...
        for entry in &store.manifest().objects {
            ...
            let recipe_contract = match entry.kind {
                ...
                RenderObjectKind::StandardHonor | RenderObjectKind::BondsHonor => {
                    sekai_profile_renderer::render_object::HONOR_RENDER_OBJECT_CONTRACT
                }
                RenderObjectKind::CardMember => {
                    sekai_profile_renderer::profile_compositor::DECK_ART_VARIANT_CONTRACT
                }
                RenderObjectKind::Component => {
                    sekai_profile_renderer::profile_compositor::GENERAL_BASE_CONTRACT
                }
            };
...
fn run_plan_desired_catalog(args: &[String]) -> Result<(), String> {
    ...
    let deck_contract = sekai_profile_renderer::profile_compositor::DECK_ART_VARIANT_CONTRACT;
    ...
    for texture in texture_sealed.objects.iter().filter(|object| {
        ...
        let (key, _) =
            sekai_profile_renderer::profile_compositor::deck_art_variant_identity_from_source(
    ...
    let honor_contract = sekai_profile_renderer::render_object::HONOR_RENDER_OBJECT_CONTRACT;
...
fn run_deck_art_store(args: &[String]) -> Result<(), String> {
    ...
    for entry in &source.manifest().objects {
        ...
        let (key, _) =
            sekai_profile_renderer::profile_compositor::deck_art_variant_identity(object);
    ...
    let mut identity = Sha256::new();
    identity
        .update(sekai_profile_renderer::profile_compositor::DECK_ART_VARIANT_CONTRACT.as_bytes());
    ...
    for (expected_key, source_key) in planned {
        ...
        let built = sekai_profile_renderer::profile_compositor::build_deck_art_variant_simd(object)
            .map_err(|error| error.to_string())?;
...
mod tests {
    ...
    fn compiled_resources_resolve_static_and_normalized_cache_paths() {
        ...
        let static_resource = CompiledResourceRequest {
            ...
            use_kind: sekai_profile_renderer::compiled_profile::CompiledResourceUseKind::Image,
    ...
    fn desired_catalog_planner_adds_new_texture_and_deck_variant_without_profiles() {
        ...
        let header = DesiredRenderObjectCatalog {
            schema:
                sekai_profile_renderer::render_object_catalog::DESIRED_RENDER_OBJECT_CATALOG_SCHEMA
                    .into(),
            region: "cn".into(),
