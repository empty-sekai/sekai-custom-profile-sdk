...
struct RasterLayer {
    dynamic_layer_id: Option<sekai_profile_renderer_core::LayerId>,
...
type AnimationRasterGroup = (
    Option<sekai_profile_renderer_core::LayerId>,
    Vec<(sekai_profile_renderer_core::AuthoredElementKind, usize)>,
);
...
fn animation_layer_expansions(
    scene: &mut sekai_profile_renderer_core::Scene,
    dynamic_layer_ids: &[sekai_profile_renderer_core::LayerId],
    ticks: &[u64],
) -> std::collections::BTreeMap<sekai_profile_renderer_core::LayerId, [i32; 4]> {
...
fn rasterize_animation_groups(
    ...
    md: &MasterData,
    assets: Option<&AssetStore>,
    resolved_scene: Option<&sekai_profile_renderer_core::profile_scene::ResolvedProfileScene>,
    _dynamic_expansions: &std::collections::BTreeMap<
        sekai_profile_renderer_core::LayerId,
        [i32; 4],
    >,
...
fn animation_loop_period_ticks(
    timelines: impl IntoIterator<Item = sekai_profile_renderer_core::TimelineDescriptor>,
    maximum_tick: u64,
) -> Option<u64> {
            ...
            && dynamic_layer_ids.iter().all(|id| {
                scene
                    ...
                    .is_some_and(|state| {
                        matches!(
                            state.status,
                            sekai_profile_renderer_core::DynamicStatus::Settled
                                | sekai_profile_renderer_core::DynamicStatus::Held
                        )
                    })
            });
...
impl<'a> AnimationFrameCompositor<'a> {
    ...
    fn composite(
        &mut self,
        index: u32,
        scene: &sekai_profile_renderer_core::Scene,
        layers: &[RasterLayer],
...
fn grouped_layer_card(
    ...
    members: &[(sekai_profile_renderer_core::AuthoredElementKind, usize)],
) -> CustomProfileCard {
...
mod tests {
    ...
    fn dynamic_animation_groups_keep_motion_bounds_during_rasterization() {
        ...
        assert!(animation_group_uses_dynamic_bounds(Some(
            sekai_profile_renderer_core::StableId(1)
        )));
    }
    ...
    fn profile_animation_frame_starts_with_opaque_white_canvas() {
        let scene =
            sekai_profile_renderer_core::Scene::new(sekai_profile_renderer_core::SceneSource {
                scene_id: sekai_profile_renderer_core::StableId(1),
                region: "cn".into(),
    ...
    fn dirty_compositor_matches_full_redraw_with_interleaved_static_layer() {
        let layers = vec![
            ...
            solid_layer(
                ...
                2,
                2,
                Some(sekai_profile_renderer_core::StableId(1)),
            ),
    ...
    fn ordered_layer_raster_reports_atlas_identity_runs_and_realtime_edt_totals() {
        ...
        let groups: Vec<AnimationRasterGroup> = vec![(
            None,
            vec![(sekai_profile_renderer_core::AuthoredElementKind::Shape, 0)],
        )];
    ...
    fn gif_artifact_keeps_an_initially_off_canvas_dynamic_layer_and_erases_its_old_position() {
        ...
        let layers = vec![RasterLayer {
            dynamic_layer_id: Some(sekai_profile_renderer_core::StableId(7)),
    ...
    fn loop_policy_only_repeats_periodic_programs() {
        use sekai_profile_renderer_core::TimelineDescriptor;
