...
impl CustomProfileRenderer {
    ...
    pub fn render_all_layers_cropped(
        ...
        let md = self.snapshot();
        let asset_ref = self.assets.as_deref();
        render_all_layers_cropped(card, &md, asset_ref, profile, webp_quality, include_properties)
    }
...
fn collect_element_positions(card: &CustomProfileCard) -> Vec<ElementPos> {
    ...
    for (i, e) in card.texts.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "text", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.shapes.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "shape", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.card_members.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "card_member", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.stamps.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "stamp", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.others.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "other", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.bonds_honors.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "bonds_honor", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.honors.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "honor", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.collections.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "collection", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.generals.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "general", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.stand_members.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "stand_member", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.general_backgrounds.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "general_background", idx: i, original_visible: e.object_data.visible }));
    }
    for (i, e) in card.story_backgrounds.iter().enumerate() {
        raw.push((e.object_data.layer, ElementPos { etype: "story_background", idx: i, original_visible: e.object_data.visible }));
    }
...
fn make_single_element_card(base: &CustomProfileCard, pos: &ElementPos) -> CustomProfileCard {
    ...
    for e in &mut card.texts { e.object_data.visible = false; }
    for e in &mut card.shapes { e.object_data.visible = false; }
    for e in &mut card.card_members { e.object_data.visible = false; }
    for e in &mut card.stamps { e.object_data.visible = false; }
    for e in &mut card.others { e.object_data.visible = false; }
    for e in &mut card.bonds_honors { e.object_data.visible = false; }
    for e in &mut card.honors { e.object_data.visible = false; }
    for e in &mut card.collections { e.object_data.visible = false; }
    for e in &mut card.generals { e.object_data.visible = false; }
    for e in &mut card.stand_members { e.object_data.visible = false; }
    for e in &mut card.general_backgrounds { e.object_data.visible = false; }
    for e in &mut card.story_backgrounds { e.object_data.visible = false; }
    match pos.etype {
...
pub fn render_all_layers_cropped(
    ...
    for (z, pos) in positions.iter().enumerate() {
        ...
        let rendered = render_element_layer_cropped(&layer_card, md, assets, profile, webp_quality)?;
        out.push(LayerCrop {
            z,
            element_type: pos.etype.to_string(),
...
mod tests {
    ...
    fn batch_layers_match_single_method_byte_for_byte() {
        ...
        for (z, pos) in positions.iter().enumerate() {
            ...
            assert_eq!(layer.data, single.data, "z={z} ({}): WebP 字节不一致", pos.etype);
