...
fn command_control_id(binding: &CommandControlBinding) -> StableId {
    match binding {
        ...
        | CommandControlBinding::ScrollThumb { control_id } => *control_id,
    }
}
...
mod tests {
    ...
    fn basic_general_lowering_has_profile_and_locale_provenance_without_component_layers() {
        ...
        for general_type in [2, 9, 10, 17] {
            let lowered = crate::profile_scene::lower_basic_general(
                general_type,
                StableId(91),
    ...
    fn music_general_lowering_is_typed_localized_and_preserves_append_gradient() {
        ...
        for general_type in [12, 16] {
            let lowered = crate::profile_scene::lower_music_general(
                general_type,
                StableId(92),
            ...
            if general_type == 16 {
                ...
                assert_eq!(lowered.interaction_regions.len(), 3);
                assert!(lowered
                    .interaction_regions
                    .iter()
                    .all(|region| region.capabilities.contains(&"activate".into())));
                assert_eq!(
                    lowered
                        .commands
                        .iter()
    ...
    fn image_general_lowering_exposes_source_metadata_cover_uv_and_clip() {
            ...
            honor_slots: vec![
                crate::profile_scene::HonorVisualSnapshot {
                    ...
                    visual: crate::profile_scene::HonorVisualKind::Standard {
                        ...
                        progress: 0,
                        background: descriptor.clone(),
                        frame: descriptor.clone(),
                        overlay: descriptor.clone(),
                        star: descriptor.clone(),
                        star_high: descriptor.clone(),
                        live_star_on: descriptor.clone(),
                        live_star_off: descriptor.clone(),
                    },
                },
                crate::profile_scene::HonorVisualSnapshot {
                    source_field: "userProfile.honorSlots".into(),
                    ...
                    visual: crate::profile_scene::HonorVisualKind::Bonds {
                        character_ids: [1, 2],
                        backgrounds: [descriptor.clone(), descriptor.clone()],
                        characters: [descriptor.clone(), descriptor.clone()],
                        mask: descriptor.clone(),
                        frame: descriptor.clone(),
        ...
        let story = crate::profile_scene::lower_image_general(14, StableId(93), "story", &snapshot)
            .unwrap()
            .unwrap();
        let avatar =
            crate::profile_scene::lower_image_general(18, StableId(94), "avatar", &snapshot)
                .unwrap()
                .unwrap();
        assert!(!story
            .commands
        ...
        let ranks = crate::profile_scene::lower_character_rank_general(
            11,
            StableId(95),
            "ranks",
            &snapshot,
        ...
        assert_eq!(ranks.interaction_regions.len(), 3);
        for general_type in [3, 5] {
            let cards = crate::profile_scene::lower_card_general(
                general_type,
                StableId(98),
        ...
        let honors =
            crate::profile_scene::lower_honor_general(6, StableId(99), "honors", &snapshot)
                .unwrap()
                .unwrap();
        assert_eq!(honors.interaction_regions.len(), 2);
        assert!(honors.commands.iter().any(|command| matches!(
            &command.payload,
            SemanticCommandPayload::Image { alpha_mask: Some(mask), .. } if mask.key == "fixture/wide"
        )));
        assert!(!honors
            .commands
            .iter()
        ...
        let compact = crate::profile_scene::lower_character_rank_general(
            15,
            StableId(96),
            "compact-ranks",
            &snapshot,
        ...
        scrolling_story_snapshot.story_favorites = (0..12)
            ...
            .collect();
        let scrolling_story = crate::profile_scene::lower_image_general(
            14,
            StableId(97),
            "scrolling-story",
            ...
            let visual = match element.value {
                crate::profile_scene::ProfileElementRef::Honor(value) => {
                    Some(crate::profile_scene::HonorVisualSnapshot {
                        ...
                        visual: crate::profile_scene::HonorVisualKind::Standard {
                            ...
                            is_live_master: false,
                            progress: 0,
                            background: honor_asset.clone(),
                            frame: honor_asset.clone(),
                            overlay: honor_asset.clone(),
                            star: honor_asset.clone(),
                            star_high: honor_asset.clone(),
                            live_star_on: honor_asset.clone(),
                            live_star_off: honor_asset.clone(),
                        },
                    })
                }
                crate::profile_scene::ProfileElementRef::BondsHonor(value) => {
                    Some(crate::profile_scene::HonorVisualSnapshot {
                        ...
                        visual: crate::profile_scene::HonorVisualKind::Bonds {
                            character_ids: [1, 2],
                            backgrounds: [honor_asset.clone(), honor_asset.clone()],
                            characters: [honor_asset.clone(), honor_asset.clone()],
                            mask: honor_asset.clone(),
                            frame: honor_asset.clone(),
