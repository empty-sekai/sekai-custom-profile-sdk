...
pub fn prepare_profile(
    ...
    if let Some(component) = &snapshot.component {
        collect_optional_image(
            &mut resources,
            "component.challenge_avatar",
            component.challenge_avatar.as_ref(),
        );
        collect_optional_image(
            &mut resources,
            "component.player_avatar",
            component.player_avatar.as_ref(),
        );
        for (index, story) in component.story_favorites.iter().enumerate() {
            collect_optional_image(
                &mut resources,
                &format!("component.story_favorites.{index}"),
                Some(&story.image),
            );
        }
        for (index, rank) in component.character_ranks.iter().enumerate() {
            collect_optional_image(
                &mut resources,
                &format!("component.character_ranks.{index}"),
                Some(&rank.avatar),
            );
        }
        for (index, card) in component.deck_members.iter().enumerate() {
            collect_optional_image(
                &mut resources,
                &format!("component.deck_members.{index}"),
                Some(&card.image),
            );
        }
        if let Some(card) = &component.leader_card {
            collect_optional_image(&mut resources, "component.leader_card", Some(&card.image));
        }
        for (index, honor) in component.honor_slots.iter().enumerate() {
            collect_honor_descriptors(
                &mut resources,
                &format!("component.honor_slots.{index}"),
                honor,
            );
        }
    }
...
fn collect_optional_image(
    resources: &mut BTreeMap<String, ProfileResourceRequest>,
    role: &str,
    image: Option<&ComponentImageSnapshot>,
) {
    if let Some(descriptor) = image.and_then(|image| image.descriptor.as_ref()) {
        collect_descriptor(resources, role.into(), descriptor);
    }
}
    ...
    match &honor.visual {
        HonorVisualKind::Standard {
            ...
            for (name, value) in [
                ...
                ("star", star),
                ("star_high", star_high),
                ("live_star_on", live_star_on),
                ("live_star_off", live_star_off),
            ] {
                add(name, value);
            }
        }
        HonorVisualKind::Bonds {
            ...
            add("background.0", &backgrounds[0]);
            add("background.1", &backgrounds[1]);
            add("character.0", &characters[0]);
            add("character.1", &characters[1]);
            add("mask", mask);
            add("frame", frame);
...
fn build_component(
    ...
    let image = |field: &str,
        ...
        source_id,
        descriptor: Some(descriptor(key, size, table, id, metadata)),
    };
    let static_image = |field: &str,
        ...
        source_field: field.into(),
        source_id,
        descriptor: Some(descriptor_in("static", key, size, table, id, metadata)),
    };
    ...
    Ok(ProfileComponentSnapshot {
        locale: locale.into(),
        region_fonts: BTreeMap::from([(1, font)]),
        localized_text,
...
fn standard_honor_visual(
    ...
    let (frame_namespace, frame_key) = resolved
        .frame_name
        .as_ref()
        .map(|name| {
            (
        ...
        .unwrap_or_else(|| ("static", format!("honor/frame_degree_{size_char}_{rarity}")));
...
fn descriptor(
    ...
    descriptor_in("assets", key, fallback, table, id, metadata)
}
