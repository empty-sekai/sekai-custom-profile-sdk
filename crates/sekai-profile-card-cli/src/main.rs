...
use sekai_profile_renderer::types::{CustomProfileCard, UserCustomProfileCard};
    ...
    match format {
        "jpeg" | "jpg" => {
            #[cfg(feature = "jpeg-turbo")]
            {
                ...
                sekai_profile_renderer::jpeg_turbo::encode_rgba(
                    &output.rgba,
                    output.width,
                    output.height,
                    90,
...
fn encode_candidate_png(
    output: sekai_profile_renderer::renderer::FullCardSdfExecutionOutput,
) -> Result<Vec<u8>, String> {
    ...
    for pixel in samples.chunks_exact_mut(4) {
        ...
        if alpha != 0 && alpha != u8::MAX {
            for channel in 0..3 {
                pixel[channel] = sekai_profile_renderer::codec::unpremultiply_channel_like_skia(
                    pixel[channel],
                    alpha,
                );
            }
    ...
    sekai_profile_renderer::codec::png::encode_rgba(output.width, output.height, &samples)
...
fn main() -> ExitCode {
    ...
    for manifest in &args.text_atlases {
        let atlas = match sekai_profile_renderer::sdf::atlas::MappedSdfAtlas::open(manifest) {
    ...
    if let Some(manifest) = &args.shape_atlas {
        let atlas =
            match sekai_profile_renderer::sdf::shape_atlas::MappedShapeSdfAtlas::open(manifest) {
                Ok(atlas) => Arc::new(atlas),
                Err(err) => {
    ...
    if let Some(manifest) = &args.render_objects {
        let store =
            match sekai_profile_renderer::render_object::MappedRenderObjectStore::open(manifest) {
