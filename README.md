...
仓库采用 AGPL-3.0-only。browser npm package 另含 `crates/allium-renderer-wasm/LICENSE-EXCEPTION` 中的有限 browser linking exception。修改 renderer、服务端使用和非浏览器使用仍受完整 AGPL 约束，包括网络交互场景下的源代码提供义务。
